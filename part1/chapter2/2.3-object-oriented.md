...
看Scala源码不难发现，函数本质就是new了一个Function0~Function22其中之一**接口的实例**，数字取决于输入参数的个数。函数中有一个apply方法，调用写`f(10)`和`f.apply(10)`一样。
...
若是定义一个**方法**然后传入，实际上还是把方法转成了函数传入，生成的函数再去**调用**这个方法起作用。`方法名 _`就可以将方法转换为函数，例如`methodName _`。
